 ...
 * Description      : Gets the random index among 4/5 of the greatest variances
 *                    out of all the vectors
 * Parameter(s)     : subset - The data set to find the random varianced
 *                             index
 * Return Value     : The index that is randomly selected among the 4/5 of the max variance
 */
template<class Label, class T>
size_t ran_variance_index(DataSet<Label, T> & subset)
{
    ...
    T pivot = selector(var, (size_t)(var.size()-4));
    vector<double> max_var;
    for (size_t i = 0; i < dimension; i++) {
        if (var[i] >= pivot) {//0.8*var[maxIndex]) {
            max_var.push_back(i);
        }
    }
