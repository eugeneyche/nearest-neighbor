...
public:
    ...
    void s_multi_kd_tree(double ll, int n) {
        stringstream dir;
        for (int i=1; i<=n; i++) {
            dir << base_dir_ << "/kd_tree" << i << "_" << setprecision(2) << ll;
            KDTree<Label, T> tree ((size_t)(ll * (*trn_st_).size()), *trn_st_);
            ofstream tree_out (dir.str());
            tree.save(tree_out);
            tree_out.close();
    ...
    void generate_multi_kd_trees() {
        s_multi_kd_tree(min_leaf, multi);
    }
    ...
    /*void s_multi_bsp_tree(double ll, int n)
    {
        stringstream dir;
        for (int i=1; i<=n; i++) {
            dir << base_dir_ << "/bsp_tree" << i << "_" << setprecision(2) << ll;
            BSPTree<Label, T> tree ((size_t)(ll * (*trn_st_).size()), *trn_st_);
            ofstream tree_out (dir.str());
            tree.save(tree_out);
            tree_out.close();
    ...
    void s_multi_kd_tree_data(double ll, string * result, int n)
    {
        ...
        for (size_t i = 0; i < (*tst_st_).size(); i++) {
            ...
            for (int j=1; j<=n; j++) {
                dir << base_dir_ << "/kd_tree" << j << "_" << setprecision(2) << min_leaf;
                string test = dir.str();
                ifstream tree_in (dir.str());
                KDTree<Label, T> tree (tree_in, *trn_st_);
                DataSet<Label, T> subSet = (*trn_st_).subset(tree.subdomain((*tst_st_)[i], (size_t)(ll * (*trn_st_).size())));
    ...
    {
        ofstream dat_out (out_dir + "/multi_kd_tree.dat");
        dat_out <<  setw(COL_W) << "leaf";
        dat_out <<  setw(COL_W) << "error rate";
        dat_out <<  setw(COL_W) << "true nn";
        dat_out <<  setw(COL_W) << "subdomain";
        ...
        string r [l_len];
        for (size_t i = 0; i < l_len; i++) {
            t[i] = thread(&Test::s_multi_kd_tree_data, this, l[i], &(r[i]), multi);
        }
        for (size_t i = 0; i < l_len; i++) {
            t[i].join();
            dat_out << r[i];
    ...
    {
        ofstream dat_out (out_dir + "/bsp_tree.dat");
        dat_out <<  setw(COL_W) << "leaf";
        dat_out <<  setw(COL_W) << "error rate";
        dat_out <<  setw(COL_W) << "true nn";
        dat_out <<  setw(COL_W) << "subdomain";
    ...
    {
        ofstream dat_out (out_dir + "/bsp_spill_tree.dat");
        dat_out <<  setw(COL_W) << "leaf";
        dat_out <<  setw(COL_W) << "alpha";
        dat_out <<  setw(COL_W) << "error rate";
        dat_out <<  setw(COL_W) << "true nn";
