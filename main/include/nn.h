...
vector<T> * 
nearest_neighbor(const vector<T> * query, const DataSet<Label, T> & st)
{
...
template<class Label, class T>
DataSet<Label, T> 
k_nearest_neighbor(size_t k, vector<T> * query, DataSet<Label, T> & st)
{
...
template<class Label, class T>
DataSet<Label, T> 
c_approx_nn(double c, vector<T> * query, DataSet<Label, T> & st, 
        vector<T> * nn)
{
    map<vector<T> * , double> dist_mp;
    for (size_t i = 0; i < st.size(); i++) {
